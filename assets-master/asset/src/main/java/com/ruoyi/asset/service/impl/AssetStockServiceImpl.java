...
public class AssetStockServiceImpl implements IAssetStockService 
{
    ...
    {
    	AssetStock sc = new AssetStock();
    	sc.setYear(assetStock.getYear());
    	List<AssetStock> old = assetStockMapper.selectAssetStockList(sc);
    	if(old != null && old.size() > 0) {
    		throw new CustomException("资产统计记录已存在，您可以进行重新计算，不需要重复添加");
    	}
