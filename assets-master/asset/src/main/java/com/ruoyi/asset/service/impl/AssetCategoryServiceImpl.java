...
public class AssetCategoryServiceImpl implements IAssetCategoryService 
{
     ...
     * @return 资产分类集合
     */
    public List<AssetCategory> selectAssetCategoryTree(AssetCategory assetCategory){
    	List<AssetCategory> tree = new ArrayList<>();
    	List<AssetCategory> list = assetCategoryMapper.selectAssetCategoryList(assetCategory);
    	for(AssetCategory cate:list) {
    		if(cate.getPid() == null || cate.getPid() == 0l) {
    			tree.add(cate);
    		}
    	}
    	for(AssetCategory cate:tree) {
        	List<AssetCategory> cc = new ArrayList<>();
    		for(AssetCategory c:list) {
        		if(c.getPid() != null && c.getPid().equals(cate.getId())) {
        			cc.add(c);
        		}
        		cate.setChildren(cc);
        	}
