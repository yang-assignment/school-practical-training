...
public class AssetPurchaseServiceImpl implements IAssetPurchaseService 
{
    ...
    public int insertAssetPurchase(AssetPurchase assetPurchase)
    {
        ...
        if(assetPurchase.getCheckStatus() == null) assetPurchase.setCheckStatus("0");
        if(assetPurchase.getIsComplete() == null) assetPurchase.setIsComplete("N");
    ...
    public int checkAssetPurchase(AssetPurchase assetPurchase) {
    	...
    	sysMsgService.sendMsg("采购通知", "0", "您有新的采购任务，要求在"+DateUtils.dateTime(assetPurchase.getFinishTime())+"前完成", users, "/asset/purchase/task");
