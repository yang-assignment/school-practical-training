...
public class XdataModelController extends BaseController
{
    ...
    @ApiOperation("获取模型列表")
    @GetMapping("/data")
    public TableDataInfo data(XdataModel xdataModel)
    {
        startPage();
        xdataModel.getParams().put("offset", (PageHelper.getLocalPage().getPageNum()-1) * PageHelper.getLocalPage().getPageSize());
        xdataModel.getParams().put("rows", PageHelper.getLocalPage().getPageSize());
        PageInfo<Map> page = xdataModelService.selectXdataModelData(xdataModel);
    ...
    @ApiOperation("修改模型数据")
    @PostMapping("/data")
    public AjaxResult editData(@RequestBody XdataModel xdataModel)
    {
        return toAjax(xdataModelService.updateXdataModelData(xdataModel));
    }
