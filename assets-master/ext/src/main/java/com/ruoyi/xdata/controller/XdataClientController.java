...
public class XdataClientController extends BaseController
{
    ...
    @ApiImplicitParam(name = "id", value = "应用ID", required = true, dataType = "int", paramType = "path")
    @GetMapping("/table/{id}")
    public AjaxResult table(@PathVariable Long id)
    {
        List<Map> list = xdataClientService.selectTableList(id);
     ...
     * 查询指定表里的数据表
     */
    @ApiOperation("获取指定表里的数据")
    @GetMapping("/data")
    public TableDataInfo data(XdataClient xdataClient)
    {
        startPage();
        xdataClient.getParams().put("offset", (PageHelper.getLocalPage().getPageNum()-1) * PageHelper.getLocalPage().getPageSize());
        xdataClient.getParams().put("rows", PageHelper.getLocalPage().getPageSize());
        PageInfo<Map> page = xdataClientService.selectTableData(xdataClient);
